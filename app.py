...
IVR_SLOTS=['0 - 15 SEC','16 SEC - 30 SEC','31 SEC - 1 MIN','1.1 MIN - 2 MIN','ABOVE 2 MIN']

def ivr_wait_table(r, meta=None):
    """Single source of truth for the IVR report (Excel + dashboard).
    - Answered  : ANSWERED calls (not abandoned / IVR hangup), bucketed by queue wait time.
    - Abandoned : event == 'abandoned' only. 'IVR Hangup' calls are excluded everywhere.
    - All %ages are against the report Grand Total (answered + abandoned).
    - Within-1-min is built from the same answered buckets shown in the table and compared with answered calls."""
    meta=meta or {}
    q=r['q'].copy(); q['wait_bucket']=wait_bucket(q.queue_holdtime_sec).astype(str)
    not_ivr=~q.event_clean.eq('ivr hangup')
    ans=q[(q.disp_clean=='ANSWERED') & (~q.event_clean.eq('abandoned')) & not_ivr]
    ab=q[q.event_clean.eq('abandoned') & not_ivr]
    grand=len(ans)+len(ab)
    rows=[]
    for slot in IVR_SLOTS:
        a=int((ans.wait_bucket==slot).sum()); b=int((ab.wait_bucket==slot).sum()); g=a+b
        rows.append(dict(slot=slot,ans=a,ans_pct=a/grand if grand else 0,ab=b,ab_pct=b/grand if grand else 0,gt=g,gt_pct=g/grand if grand else 0))
    ans_total=len(ans); ab_total=len(ab)
    within1=int(sum(x['ans'] for x in rows[:3]))
    t1=meta.get('within1_target',98)/100; tA=meta.get('abandon_target',2)/100
    return dict(rows=rows,ans_total=ans_total,ab_total=ab_total,grand=grand,
        within1=within1,within1_pct=within1/ans_total if ans_total else 0,within1_target=t1,within1_target_calls=round(ans_total*t1),
        ab_pct=ab_total/grand if grand else 0,ab_target=tA,ab_target_calls=round(grand*tA))

def export_ivr_excel(r, meta=None):
    meta=meta or {}; wb,ws=_base_wb('IVR Report')
    yellow='FFF2CC'; blue='DDEBF7'; green='C6EFCE'; redfill='FFC7CE'; border=Border(*(Side(style='thin',color='000000'),)*4)
    ...
    t=ivr_wait_table(r,meta); row=3
    for x in t['rows']:
        vals=[x['slot'],x['ans'],x['ans_pct'],x['ab'],x['ab_pct'],x['gt'],x['gt_pct']]
        for c,v in enumerate(vals,1): _cell(ws,row,c,v,border=border)
        for c in [3,5,7]: ws.cell(row,c).number_format='0%'
        row+=1
    g=t['grand']
    vals=['Grand Total',t['ans_total'],t['ans_total']/g if g else 0,t['ab_total'],t['ab_total']/g if g else 0,g,1 if g else 0]
    for c,v in enumerate(vals,1): _cell(ws,row,c,v,fill=blue,font=Font(bold=True),border=border)
    for c in [3,5,7]: ws.cell(row,c).number_format='0%'
    row+=2
    blocks=[(1,'Call Answered (Within 1 Min)',t['within1_target'],t['within1_pct'],t['within1_target_calls'],t['within1']),
            (5,'Abandoned',t['ab_target'],t['ab_pct'],t['ab_target_calls'],t['ab_total'])]
    for c0,title,target,actual,tgt_calls,act_calls in blocks:
        ...
        for label,pct,calls in [('Target',target,tgt_calls),('Achieved',actual,act_calls),('Variance',actual-target,act_calls-tgt_calls)]:
            fill='FFFFFF'
            if label=='Variance':
                good=(calls>=0) if title.startswith('Call') else (calls<=0)
                fill=green if good else redfill
...
if qfile and afile:
    try:
        ...
        with t3:
            t_ivr=ivr_wait_table(r,meta)
            tbl=pd.DataFrame(t_ivr['rows']).rename(columns={'slot':'Time Slot','ans':'Answered','ans_pct':'Answered %','ab':'Abandoned','ab_pct':'Abandoned %','gt':'Grand Total','gt_pct':'Total %'})
            st.plotly_chart(px.bar(tbl,x='Time Slot',y=['Answered','Abandoned'],barmode='stack',title='Analysis of Call Waiting (IVR) Time'),use_container_width=True)
            k1,k2,k3,k4=st.columns(4)
            k1.metric('Answered',f"{t_ivr['ans_total']:,}")
            k2.metric('Abandoned (excl. IVR hangup)',f"{t_ivr['ab_total']:,}",f"{t_ivr['ab_pct']*100:.1f}%")
            k3.metric('Answered within 1 min',f"{t_ivr['within1']:,} / {t_ivr['ans_total']:,}",f"{t_ivr['within1_pct']*100:.1f}%")
            k4.metric('IVR hangups (excluded)',f"{r['ivr_n']:,}")
            st.dataframe(tbl.style.format({'Answered %':'{:.0%}','Abandoned %':'{:.0%}','Total %':'{:.0%}'}),use_container_width=True,hide_index=True)
            st.info("Abandoned calls are bucketed by queue wait time. IVR-hangup calls are excluded. All percentages are against the Grand Total (answered + abandoned).")
        with t4:
